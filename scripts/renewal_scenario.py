...
prof = st.session_state.app_db.get('profile', {}) # Corrected to app_db
...
# Initialize from Profile and Intel if empty
if 'initialized' not in ren_data:
    ren_data.update({
        "mortgage_bal": float(prof.get('m_bal', 500000.0)),
        "remaining_amort": float(prof.get('m_amort', 25.0)),
...
<div style="background-color: {OFF_WHITE}; padding: 25px; border-radius: 12px; border: 1px solid {BORDER_GREY}; border-left: 8px solid {PRIMARY_GOLD};">
    ...
    <p style="color: {SLATE_ACCENT}; font-size: 1.1em; margin-bottom: 10px;">
        <b>Instruction:</b> Input your bank's <b>Fixed Rate</b> offer in Column 1. In Column 2, define your <b>Market Outlook</b> 
        for the Variable path to compare long-term interest costs.
    </p>
    <p style="font-size: 0.95em; color: #6c757d; font-style: italic;">
        Note: Balance and Amortization are synced from your Profile. Market rates are fetched from Intel.
    </p>
</div>
""", unsafe_allow_html=True)
st.write("")

# --- 5. INPUTS (RETAINING ORIGINAL 2-COLUMN LAYOUT) ---
...
with col1:
    ...
    curr_yrs = int(ren_data.get('term_months', 60) / 12)
    m_term_yr = st.selectbox("Renewal Term (Years)", term_options, 
                             index=term_options.index(curr_yrs) if curr_yrs in term_options else 4,
...
end_r = target_r / 100
mo_drop = (start_r - end_r) / target_mo if target_mo > 0 else 0
...
for m in range(1, months + 1):
    ...
    # Variable Path Logic
    curr_v_rate = start_r - (mo_drop * (m-1)) if m <= target_mo else end_r
    ...
    data_rows.append({
        "Month": m, "V_Rate": curr_v_rate*100, "V_Pmt": pmt_var, 
        "F_Pmt": pmt_fixed, "Cum_V_Int": v_int_total, "Cum_F_Int": f_int_total
    })

df = pd.DataFrame(data_rows)
savings = f_int_total - v_int_total

# --- 7. EXECUTIVE SUMMARY (RESTORING ORIGINAL 3-COLUMN METRICS) ---
st.divider()
...
with v3:
    st.metric("Required Cuts to Break Even", f"{((var_start - fixed_offer) / 0.25):.1f} cuts")
...
with tab2:
    ...
    fig_pmt.add_trace(go.Scatter(x=df["Month"], y=[pmt_fixed]*months, name="Fixed Payment", line=dict(color=CHARCOAL, dash='dash')))
