...
for i, listing in enumerate(st.session_state.rental_listings):
    with st.expander(f"Listing #{i+1}: {listing['address'][:25]}...", expanded=(i==len(st.session_state.rental_listings)-1)):
        ...
        with r1_c1: listing['address'] = st.text_input("Property Address", value=listing['address'], key=f"addr_{i}", label_visibility="collapsed")
...
if results:
    ...
    r1.metric("DP Required", f"${top['DP']:,.0f}"); r2.metric("Gross Income", f"${top['Gross']:,.0f}"); r3.metric("Op Expenses", f"${top['OpEx']:,.0f}"); r4.metric("Annual Mtg", f"${top['Mtg']:,.0f}")
