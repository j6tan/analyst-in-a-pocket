...
# --- 3. DYNAMIC MARGINAL TAX CALCULATOR (The Upgrade) ---
...
if not aff_sec.get('initialized'):
    aff_sec.update({
        ...
        "mgmt_pct": 5.0, "is_vanc": False, "initialized": True
    })
...
with ts_col1:
    ...
    asset_province = st.selectbox("Asset Location (Province):", options=prov_options, index=prov_options.index(aff_sec.get('asset_province', 'BC')), key="aff_sec_prov_select")
    aff_sec['asset_province'] = asset_province
with ts_col2:
    use_case = st.selectbox("Use of the Second Home:", ["Rental Property", "Family Vacation Home"], index=0 if aff_sec.get('use_case') == "Rental Property" else 1, key="aff_sec_use_select")
    aff_sec['use_case'] = use_case
    is_rental = (use_case == "Rental Property")

# --- 7. CORE CALCULATION PREP (Dynamic Income) ---
...
with c_left:
    ...
    calc_rate = float(aff_sec.get('contract_rate', 4.5))
    calc_rent = float(aff_sec.get('manual_rent', 0))
...
with c_right:
    ...
    if asset_province == "BC" and not is_rental:
        st.markdown("---")
        vanc_check = st.checkbox("Property in Vancouver?", value=aff_sec.get('is_vanc', False), key="aff_sec_is_vanc")
        aff_sec['is_vanc'] = vanc_check
...
# Net Tax Impact (Monthly)
taxable_asset_income = (realized_rent * 12) - (f_tax + (f_ins * 12) + (f_strata * 12) + (f_rm * 12))
asset_tax_mo = max(0, (taxable_asset_income * (marginal_tax_rate / 100)) / 12) if is_rental else 0
...
with col_b2:
    ...
    st.table(pd.DataFrame([{"Item": "Realized Rent", "Amount": f"${realized_rent:,.0f}"}, {"Item": "OpEx, Tax & Mtg", "Amount": f"-${total_opex_mo + new_p_i + asset_tax_mo:,.0f}"}, {"Item": "Net Asset Cash Flow", "Amount": f"${asset_net:,.0f}"}]))
...
# --- 10. STRATEGIC VERDICT & RESILIENCE ---
