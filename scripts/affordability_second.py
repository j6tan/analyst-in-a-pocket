...
if "aff_second_store" not in st.session_state:
    # Set initial default values ONLY once
    init_price = 600000.0
    init_tax = init_price * default_tax_rate
    init_rent = (init_price * (scraped_yield/100)) / 12
    
    st.session_state.aff_second_store = {
        "down_payment": 200000.0,
        "target_price": init_price,
        "manual_rent": init_rent,
...
rent_offset = (store.get('manual_rent', 0) * 0.80) if is_rental else 0
...
with c_left:
    ...
    # We check if the price changed to offer a reset, but we don't force it
    new_target_price = st.number_input(f"Maximum Buying Power (Qualified: ${max_buying_power:,.0f})", 
                                           value=min(float(store['target_price']), max_buying_power), 
                                           max_value=max_buying_power, step=5000.0)
    
    # If the user changes the price, update it in the store
    if new_target_price != store['target_price']:
        store['target_price'] = new_target_price
        # Optional: Auto-update taxes/rent only on price change if desired, 
        # but to keep user overrides, we leave them alone unless requested.
    ...
    if is_rental:
        ...
        st.info(f"ℹ️ Secondary Home: Income must support 100% of costs in {asset_province}.")
